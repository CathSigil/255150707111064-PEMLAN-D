...
public class MaxMinTeamFutsal {
    public static void main(String[] args) {
        ...
        String[] namaTim = {"Tim A", "Tim B"};
        ArrayList[] semuaTim = {timA, timB};

        for (int i = 0; i < semuaTim.length; i++) {
            ArrayList<Player> tim = semuaTim[i];

            Player maxTinggi = Collections.max(tim, (a, b) -> a.tinggiBadan - b.tinggiBadan);
            Player minTinggi = Collections.min(tim, (a, b) -> a.tinggiBadan - b.tinggiBadan);
            Player maxBerat  = Collections.max(tim, (a, b) -> a.beratBadan  - b.beratBadan);
            Player minBerat  = Collections.min(tim, (a, b) -> a.beratBadan  - b.beratBadan);

            System.out.println("=== " + namaTim[i] + " ===");
            System.out.println("Max Tinggi Badan : " + maxTinggi.tinggiBadan + " cm -> " + maxTinggi);
            System.out.println("Min Tinggi Badan : " + minTinggi.tinggiBadan + " cm -> " + minTinggi);
            System.out.println("Max Berat Badan  : " + maxBerat.beratBadan   + " kg -> " + maxBerat);
            System.out.println("Min Berat Badan  : " + minBerat.beratBadan   + " kg -> " + minBerat);
            System.out.println();
        }
    }
}